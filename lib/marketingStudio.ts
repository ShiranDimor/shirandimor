...
- video_script: אם content_type הוא reel - סקריפט קצר לסרטון (עד כ-45 שניות): מה נאמר/קורה על המסך, מבנה של קאט-קאט (לא פסקה רציפה), כולל הצעה למשפט פתיחה חזק ב-2-3 השניות הראשונות, ומשפט סגירה שמזכיר בקצרה את הניסיון החינמי (ר' סעיף CTA למטה). אם content_type הוא feed_post או story - השאר מחרוזת ריקה.
...
לגבי קישור/CTA: חובה בכל פוסט, בלי יוצא מן הכלל ולא משנה content_type (פוסט פיד, רילס או סטורי) - לסיים את ה-caption בשורה נפרדת בסוף, טבעית ולא לחוצה, שמפנה בפועל לקישור shirandimor.com/trial כהזמנה להתנסות בניסיון חינמי של 7 ימים.
...
לגבי קישור/CTA: התמונה שמצורפת לפוסט הזה כוללת כפתור גרפי קבוע (לא לחיץ בפועל - זו רק תמונה) שכתוב עליו "7 ימי ניסיון ללא עלות". חובה - בכל פוסט, גם על עסקה מרוויחה וגם על עסקה מפסידה - לסיים את ה-caption בשורה נפרדת בסוף, טבעית ולא לחוצה, שמזכירה בקצרה את האפשרות להתנסות ומפנה בפועל לקישור shirandimor.com/trial (זה קישור אמיתי שאפשר ללחוץ עליו כשהוא בטקסט הפוסט, בניגוד לכפתור שבתמונה). בעסקה מרוויחה הניסוח יכול להיות מוזמנים לבדוק/להתרשם; בעסקה מפסידה יותר מתאים ניסוח כמו "מי שרוצה להבין איך בונים תהליך כזה" - בלי להישמע כמו מכירה אגרסיבית אחרי הודאה בהפסד.
...
זכור/י לכלול בסוף ה-caption שורת CTA לניסיון החינמי (shirandimor.com/trial), לפי ההנחיה למעלה - חובה גם ${input.pct >= 0 ? 'בעסקה מרוויחה' : 'בעסקה מפסידה'} הזו.
...
כתיבת ה-hook - זו השורה הכי חשובה בכל הפוסט, כי היא גם הכותרת הגדולה שמוצגת בגרפיקה של הפוסט עצמו (הדבר הראשון שעין של מישהו/י שגולל/ת עוצרת עליו). תחשוב/י כמו קופירייטר מעולה שכותב הוק שעוצר גלילה - לא כמו קלישאת "גורו" (בלי "פיצחתי את הקוד", בלי הבטחות). מה שכן עובד: משפט אחד חד וממוקד, ניגוד/מתח אמיתי (למשל "רוב האנשים חושבים X, אבל בפועל Y"), ספציפיות קונקרטית במקום ניסוח כללי, או פער סקרנות כן (משפט שמעורר שאלה אמיתית בלי לענות עליה מיד) - הכול מבוסס אך ורק על מה ששירן בפועל אמרה, לא המצאה. משפט קצר וחד תמיד עדיף על משפט ארוך ומוסבר.

highlightPhrase - מתוך ה-hook עצמו, בחר/י את הביטוי הכי חד וחזק (2-5 מילים, verbatim בדיוק כפי שמופיע בתוך ה-hook - חייב להיות תת-מחרוזת מדויקת שלו) שראוי להדגשה חזותית מיוחדת בגרפיקה (כמו סימון טוש מרקר) - החלק שהכי "תופס" את העין. קריטי: הביטוי חייב להתחיל ולהסתיים בגבול מילה שלמה (אחרי רווח/בתחילת המשפט, ולפני רווח/סימן פיסוק) - אסור לחתוך אמצע מילה, וגם לא לחתוך בין אות יחס דבוקה (כמו ש-/ו-/ב-/ל-/מ-/כ-/ה-) למילה שהיא מחוברת אליה בלי רווח - אם יש ספק, כלול/י את האות הדבוקה בתוך ה-highlightPhrase ולא תתחיל בדיוק אחריה.
...
לגבי קישור/CTA: חובה לסיים את ה-caption בשורה נפרדת בסוף, טבעית ולא לחוצה (לא נשמעת כמו מכירה), שמפנה בפועל לקישור shirandimor.com/trial כהזמנה להתנסות בניסיון חינמי של 7 ימים - זה תקף לכל פוסט בלי יוצא מן הכלל.

ענה אך ורק ב-JSON תקין (בלי טקסט לפני/אחרי, בלי markdown code fence):
{"hook": "כותרת פתיחה קצרה וחדה - עד 60 תווים, כי היא גם מוצגת בגרפיקה בגופן גדול", "highlightPhrase": "2-5 מילים מתוך ה-hook בדיוק, להדגשה", "caption": "טקסט הפוסט המלא", "hashtags": "5-8 האשטגים רלוונטיים מופרדים ברווח", "topicSummary": "תקציר קצר (עד 12 מילים) של איזו נקודה נבחרה, לצורך תיעוד פנימי בלבד"}`;
...
export async function generateFromWhatsapp(params: {
  ...
  brandVoice: BrandVoice;
  avoidHooks: string[];
}): Promise<{ hook: string; caption: string; hashtags: string; topicSummary: string; highlightPhrase: string }> {
  const voiceParts: string[] = [];
...
${clippedText}`;
  ...
  return {
    ...
    highlightPhrase: stripMarkdown(String(parsed.highlightPhrase || '')),
  };
}
