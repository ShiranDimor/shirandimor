...
לגבי קישור/CTA: התמונה שמצורפת לפוסט הזה כוללת כפתור גרפי קבוע (לא לחיץ בפועל - זו רק תמונה) שכתוב עליו "7 ימי ניסיון ללא עלות". אם קיבלת הנחיה למטה לכלול קריאה לפעולה - סיים את ה-caption בשורה נפרדת, טבעית ולא לחוצה, שמזכירה את הניסיון החינמי ומפנה בפועל לקישור shirandimor.com/trial (כי זה קישור אמיתי שכן אפשר ללחוץ עליו כשהוא בטקסט הפוסט, בניגוד לכפתור שבתמונה). אם לא קיבלת הנחיה כזו - אל תוסיף קישור בכלל.
...
${input.pct >= 0 ? '\nזו עסקה מרוויחה - כלול/י בסוף ה-caption שורת CTA לניסיון החינמי, לפי ההנחיה למעלה.' : ''}
